# файл маршрутов, обрабатывает обращения по маршруту с помощью функций

# импортируем из пакета app нашу переменную app (наше приложение) и переменную db (переменная БД)
from app import app, db
# импортируем из файла models.py наши таблицы
from app.models import Car, Rent
# импортируем из flask функии: render_template - для отрисовки html-страниц, reguest - для выполнения запросов к БД...
...
def index():
    ...
    cars = Car.query.all()
    # пустой список в который будем заносить детальную информацию каждого авто
    car_list = []
...
# маршрут страницы Добавления нового автомобиля
...
def create_auto():
    ...
    if request.method == 'POST':
        ...
        db.session.add(Car(name_auto=name_auto, describe=describe, rent_price=rent_price, transmission=transmission, 
            img_url=img_url, img_url2=img_url2, img_url3=img_url3, img_url4=img_url4))
...
# маршурт для Детальной страницы, где можно просмотреть данные, изменить данные, арендовать авто и отменить аренду
# удалить авто из БД
...
def auto_detail(car_id):
    ...
    if request.method == 'POST':
        ...
        if new_name: 
            car.name_auto = new_name
        if new_describe: 
            car.describe = new_describe
        if new_rent_price: 
            car.rent_price = new_rent_price
        if new_img_url: 
            car.img_url = new_img_url
        if new_img_url2: 
            car.img_url2 = new_img_url2
        if new_img_url3: 
            car.img_url3 = new_img_url3
        if new_img_url4: 
            car.img_url4 = new_img_url4
    ...
    context = {
        ...
        'ch_value': ch_value,
        'img_url': car.img_url,
        'img_url2': car.img_url2,
        'img_url3': car.img_url3,
        'img_url4': car.img_url4,
        'list_price': list_price,
...
# маршрут для Аренды автомобиля, обрабатывает действие кнопок: 'Арендовать' и 'Освободить' на детальной странице
...
# маршрут для Удаления автомобиля из БД, обрабатывает действие кнопки: 'Удалить'
